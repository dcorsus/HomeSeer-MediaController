...
Module Main
    ...
    Private sIp As String = LoopBackIPv4Address
    ...
    Public Function AddInstance(InstanceName As String) As HSPI
        ...
        Try
            lhost.Connect(tIFACE_NAME, InstanceName)
...
TryAgain:
        ...
        Catch ex As Exception
            Console.WriteLine("Cannot connect attempt " & Attempts.ToString & ": " & ex.Message, LogType.LOG_TYPE_ERROR)
        ...
        Try
            ...
            Log("Starting to shut down ......MyShutdownRequest = " & MyShutDownRequest.ToString & " and bShutDown = " & bShutDown.ToString, LogType.LOG_TYPE_WARNING)
            If Not bShutDown Then
                Try
                    ...
                    For Each DE In AllInstances
                        ...
                        Dim ih As InstanceHolder = DE.Value
                        Try
                            If ih.hspi IsNot Nothing Then
                                Log("Shutting down  = " & ih.hspi.instance, LogType.LOG_TYPE_WARNING)
                            End If
                        Catch ex As Exception
                        End Try
                        Try
            ...
            End
        Catch ex As Exception
            Console.WriteLine("Cannot connect(2): " & ex.Message, LogType.LOG_TYPE_ERROR)
            wait(2)
            End
            Return
        End Try
